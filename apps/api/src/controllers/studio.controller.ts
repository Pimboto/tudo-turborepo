...
export class StudioController {
  ...
  static async create(req: AuthenticatedRequest, res: Response) {
    ...
    res.status(201).json(
      successResponse(studio, 'Studio created successfully')
    );
  }
  ...
  static async getAnalytics(req: AuthenticatedRequest, res: Response) {
    ...
    // Get analytics
    const [bookingStats, revenueStats, popularClasses, peakHours] = await Promise.all([
      // Booking stats
    ...
    res.json(successResponse({
      ...
      revenue: {
        ...
        averagePerBooking: revenueStats._count > 0 
